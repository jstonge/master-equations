<!--
 ...
 <script>
    ...
    $: yRange = $yScale.range();
    // If y-range is [0, height], the midpoint is height/2
    // (but if the scale is reversed, the “top” might be the larger value, so we average)
  ...
  <g class="axis y-axis">
    ...
    {#if axisTitle}
        <text
          class="y-axis-title"
          text-anchor="middle"
          transform={`translate(${x1-titleOffset}, ${yCenter}) rotate(-90)`}
        >
        {axisTitle}
      </text>
    {/if}
